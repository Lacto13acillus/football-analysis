...
def interpolate_ball_positions(ball_positions: List[Dict[int, Dict[str, Any]]]) -> List[Dict[int, Dict[str, Any]]]:
    """
    Fill missing ball bboxes via per-coordinate interpolation.
    Input format:
      ball_positions[frame] = { 1: {'bbox':[x1,y1,x2,y2], 'confidence':...} } OR {}
    Output preserves same format.
    WHY:
    - Ball detection may drop for a few frames due to motion blur or occlusion.
    - Interpolation reduces false pass rejection in validate_ball_movement().
    """
    ...
    for f in range(n):
        ...
        if bb is None:
            xs1.append(np.nan); ys1.append(np.nan); xs2.append(np.nan); ys2.append(np.nan)
        else:
            xs1.append(float(bb[0])); ys1.append(float(bb[1])); xs2.append(float(bb[2])); ys2.append(float(bb[3]))
    ...
    for f in range(n):
        ...
        if np.isnan(x1) or np.isnan(y1) or np.isnan(x2) or np.isnan(y2):
            out.append({})
            continue
        out.append({1: {"bbox": [float(x1), float(y1), float(x2), float(y2)], "confidence": 1.0}})
    return out